...
module.exports = pgliteConnector;
var _pglite = require("@electric-sql/pglite");
function pgliteConnector(opts = {}) {
  ...
  function getClient() {
    ...
    _client = new _pglite.PGlite(opts);
    return _client;
  }
  ...
  return {
    ...
    prepare(sql) {
      const stmt = {
        ...
        async all(...params) {
          const result = await query(this._sql, params.length > 0 ? params : this._params);
          return result.rows;
        },
        async run(...params) {
          const result = await query(this._sql, params.length > 0 ? params : this._params);
