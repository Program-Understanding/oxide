...
def subprocess(path,z3_timeout,out_path):
    ...
    for state in simgr.stashes["deadended"]:
        #only append if there's constraints
        if state.solver.constraints:
            results['deadends'].append(state.solver.constraints)
    #capture the constraints from the lowmem states
    for state in simgr.stashes["lowmem"]:
        #only append if there's constraints
        if state.solver.constraints:
            results['lowmem'].append(state.solver.constraints)
...
#main functionality,
#grab the path of the file, and where we should send our output
path = sys.argv[1]
out_path = sys.argv[2]
oid = sys.argv[3]
