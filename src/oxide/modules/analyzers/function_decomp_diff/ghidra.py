# -*- coding: utf-8 -*-
"""Retrieve decompiled function lines from Ghidra (decmap parsing, function lookup)."""
...
def retrieve_function_decomp_lines(oid: str, func_addr: Any) -> Optional[List[str]]:
    ...
    if not func_key:
        # Function exists in ghidra_disasm but has no decompile entry — not a failure.
        return []
    ...
    for _block_id, block in func_blocks.items():
        ...
        for raw in lines:
            ...
            # Prefer "lineno: code" format to preserve indentation after the colon.
            try:
...
def strip_decmap_prefix_if_present(lines: List[str]) -> List[str]:
    """Strip leading 'lineno:' prefixes if the list looks like raw decmap output."""
    if not lines:
        return lines
