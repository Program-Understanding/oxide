...
opts_doc = {"version": {"type": int, "mangle": True, "default": -1},
            ...
            "script": {"type": str, "mangle": True, "default": "ghidra_json_extract.java"},
...
def process(oid: str, opts: dict) -> bool:
    ...
    tmp_json = os.path.join(api.scratch_dir, f"GhidraScript-{multiprocessing.current_process().name}.json")
    ...
    try:
        ...
        if output:
            logger.info("Ghidra output:\n%s", output)
    except subprocess.CalledProcessError as e:
        output = e.output
        logger.warning("Ghidra command failed with exit code %s:\n%s", e.returncode, e.output)
    if os.path.exists(tmp_json):        
        with open(tmp_json, "r") as f:
            result = json.loads(f.read())
            
    else:
        logger.warning("No output file was generated by Ghidra script. Ghidra output:\n%s", output)
        return False
